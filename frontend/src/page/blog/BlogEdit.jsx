...
function BlogEdit() {
    ...
    const blogData = async () => {
        try {
            const res = await fetch(`http://localhost:3000/blog/edit/${blog_id}`, {
                credentials: "include"
            })
            const getdata = await res.json()
             if (!getdata.blog) {
            handlerror("Blog data not found");
            return;
        }
            setPreview(`http://localhost:3000${getdata.blog.featureImage}`)
            setValue("title", getdata.blog.title)
            setValue("slug", getdata.blog.slug)
            setValue("content", getdata.blog.content)
            setValue("category", getdata.blog.category._id)
    ...
    const onSubmit = async (blogdata) => {
        try {
            ...
            const res = await fetch(`http://localhost:3000/blog/update/${blog_id}`, {
                method: "PUT",
                credentials: "include",
                body: formData
            })
